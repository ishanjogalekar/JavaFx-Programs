...
public class Mypage extends Application{
    public void start(Stage s){
        ...
        EventHandler<MouseEvent>() {
            public void handle(MouseEvent e){
                ...
                try{
                    ...
                    String pass = "Pass123";//password
                    Connection con =DriverManager.getConnection(conURL, user, pass);
                    Statement stmt=con.createStatement();
 
                    // SQL Query for database
                    String Qs="insert into MyFamilyInfo values('"+a1+"','"+a2+"', '"+a3+"', '"+a4+"', '"+a5+"','"+a6+"','"+a7+"') ";
                ...
                try {
                    ...
                    Connection con =DriverManager.getConnection(conURL, user, pass);
                    Statement stmt = con.createStatement();
                    ResultSet rs = stmt.executeQuery("select *from MyFamilyInfo");
                    while (rs.next()) {
        ...
        s.setScene(scene);
        s.setTitle("My Page Application");
        s.show();
    }
    //Main Method
